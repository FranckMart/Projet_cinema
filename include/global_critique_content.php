...
?>
    ...
    <div id="title-intro-critique">
        <a href="#choice_content">
            <h2>Veuillez choisir le type de média</h2>
        </a>
    ...
    <article id="global_content_center">
        <nav id="content-nav-genre">
            <div class="col-12">
                ...
                ?>
                    <a href="global_critique.php?p=<?php echo $i; ?>#genre_content" class="btn btn-outline-secondary" role="button" aria-pressed="true"><?php echo $i; ?></a>
                <?php
                }
                ?>
            </div>
            ...
            <?php
                    ...
                    /*  TEST de récupération de tout les films trié par un genre 
            
                    // je stocke ma requete dans une variable que je vais utiliser plus tard 
                    //$sqlApprenants = "SELECT * FROM `stagiaire` AS s JOIN `utilisateur` AS u ON (s.`stagiaire_utilisateur_id` = u.`utilisateur_id`) WHERE s.`stagiaire_formation_id` = 1 ORDER BY s.`stagiaire_prenom` ASC LIMIT ".$offset.",".$nbCardPerPage.";";
                    $movieSelect = $bdd->prepare("SELECT news_id ,news_image,news_video,news_titrePresentation,news_titreContenu,news_textPresentation,news_textContenu,news_textConclusion,news_editeur,news_date FROM news WHERE news_id = ?");
                    // j'envoie la requete au serveur et je stocke son retour dans une autre variable
